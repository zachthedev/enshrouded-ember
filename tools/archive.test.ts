 ...
 */
function container(line: string, options: { magic?: string; relative?: number } = {}): Uint8Array {
...
describe('archiveClient', () => {
  ...
  test("the endpoint is the configured account's R2 host", () => {
    expect(signed().host).toBe(`${DESTINATION.accountId}.r2.cloudflarestorage.com`);
  });

  test('the path is the configured bucket and the manifest key', () => {
    expect(signed().pathname).toBe(`/${DESTINATION.bucket}/build/2174935030716737236/enshrouded_server.exe`);
  });
  ...
  test('no environment variable can repoint the bucket or the account', () => {
    ...
    expect(run.stdout.toString().trim()).toBe(
      `${DESTINATION.accountId}.r2.cloudflarestorage.com` + `/${DESTINATION.bucket}/build/1/enshrouded_server.exe`,
    );
  });
...
describe('credentials', () => {
  ...
  ] as const)('the %s token is read from its own two names', (access, id, secret) => {
  ...
  test('an empty environment names every variable that is absent', () => {
    ...
    try {
      ...
      expect(missing).toEqual(['R2_ARCHIVE_READ_ACCESS_KEY_ID', 'R2_ARCHIVE_READ_SECRET_ACCESS_KEY']);
  ...
  test('the refusal carries no value from any environment in reach', () => {
    ...
    try {
      ...
      const seen = [
        ...
        JSON.stringify(own),
        JSON.stringify(own.map((name) => (error as unknown as Record<string, unknown>)[name])),
        String(error),
      ].join('\n');
  ...
  test.each([
    ...
    try {
      ...
      expect((error as MissingCredentialError).missing).toEqual(['R2_ARCHIVE_READ_SECRET_ACCESS_KEY']);
    }
  });
  ...
  test('the write names do not satisfy a read', () => {
    ...
    expect(() => credentials('read', writeOnly)).toThrow(MissingCredentialError);
  });
});
...
describe('compareDigests', () => {
  ...
  test('a file that matches produces no problem', () => {
    expect(compareDigests({ 'enshrouded_server.exe': expected['enshrouded_server.exe'] }, expected)).toEqual([]);
  });

  test.each([
    ['absent', null, ['is absent']],
    ['one byte different', { bytes: 3, sha256: `${ABC_SHA256.slice(0, 63)}e` }, ['hashes to']],
    ['truncated', { bytes: 2, sha256: ABC_SHA256 }, ['is 2 bytes, and the record says 3']],
  ])('a file that is %s is reported', (_name, actual, fragments) => {
    const problems = compareDigests({ 'enshrouded_server.exe': actual }, expected);
    expect(problems).not.toHaveLength(0);
    for (const fragment of fragments) {
      expect(problems.map((problem) => problem.detail).join(' ')).toContain(fragment);
    }
  });
  ...
  test('every file the row names is checked', () => {
    ...
    const problems = compareDigests({ 'enshrouded_server.exe': expected['enshrouded_server.exe'] }, two);
    expect(problems.map((problem) => problem.fileName)).toEqual(['enshrouded_server.kfc']);
  });
});
...
describe('fetchedManifests', () => {
  ...
  test("another depot's manifest is not taken", async () => {
    const dir = await sandbox();
    await Bun.write(join(dir, 'steamapps', 'appmanifest_2278520.acf'), acf({ '1004': '7604377918839582995' }));
  ...
  test('the DepotDownloader cache names the manifest in its file name', async () => {
    const dir = await sandbox();
    await Bun.write(join(dir, '.DepotDownloader', '2278521_5177045887918896292.manifest'), 'binary');
  ...
  test('a directory carrying both records yields both', async () => {
    const dir = await sandbox();
    await Bun.write(join(dir, 'steamapps', 'appmanifest_2278520.acf'), acf({ '2278521': '2174935030716737236' }));
    await Bun.write(join(dir, '.DepotDownloader', '2278521_2174935030716737236.manifest'), 'binary');
    expect((await fetchedManifests(dir, 2278520, 2278521)).map((one) => one.by)).toEqual([
      'SteamCMD',
      'DepotDownloader',
    ]);
  });
  ...
  test('two cached manifests for one depot both yield', async () => {
    ...
    for (const gid of ['2174935030716737236', '5177045887918896292']) {
      await Bun.write(join(dir, '.DepotDownloader', `2278521_${gid}.manifest`), 'binary');
    }
    expect((await fetchedManifests(dir, 2278520, 2278521)).map((one) => one.manifestId).sort()).toEqual([
      '2174935030716737236',
      '5177045887918896292',
    ]);
  });
  ...
  test('a cached manifest for another depot is not taken', async () => {
    const dir = await sandbox();
    await Bun.write(join(dir, '.DepotDownloader', '1004_7604377918839582995.manifest'), 'binary');
    expect(await fetchedManifests(dir, 2278520, 2278521)).toEqual([]);
  });
  ...
  test("a second application's manifest is ignored", async () => {
    const dir = await sandbox();
    await Bun.write(join(dir, 'steamapps', 'appmanifest_228980.acf'), '"AppState"\n{\n\t"appid"\t\t"228980"\n}\n');
    expect(await fetchedManifests(dir, 2278520, 2278521)).toEqual([]);
  });
...
describe('the record command, driven end to end', () => {
  ...
  const fetched = async (manifest: string): Promise<string> => {
    ...
    await Bun.write(join(dir, 'steamapps', 'appmanifest_2278520.acf'), acf(manifest));
    return dir;
  };
  ...
  const record = (dir: string, manifestId: string, recordPath: string, extra: string[] = []) =>
    Bun.spawnSync({
      cmd: [
        process.execPath,
        'run',
  ...
  test('a fetch that wrote nothing is refused', async () => {
    const dir = await sandbox();
    await Bun.write(join(dir, 'steamapps', 'appmanifest_2278520.acf'), acf('2174935030716737236'));
    const recordPath = join(await sandbox(), 'build-digests.jsonl');
    const run = record(dir, '2174935030716737236', recordPath);
    expect(run.exitCode).toBe(1);
  ...
  test('a directory carrying no fetch evidence is refused, and records nothing', async () => {
    ...
    const run = record(dir, '2174935030716737236', recordPath);
    expect(run.exitCode).toBe(1);
    expect(run.stdout.toString()).toContain('carries no record of which manifest it came from');
  ...
  test('--unattested records a build with no evidence, and says so', async () => {
    ...
    await Bun.write(join(dir, 'enshrouded_server.kfc'), 'abc');
    const recordPath = join(await sandbox(), 'build-digests.jsonl');
    const run = record(dir, '2174935030716737236', recordPath, ['--unattested']);
    expect(run.exitCode).toBe(0);
  ...
  test('the flags win over the container', async () => {
    ...
    const recordPath = join(await sandbox(), 'build-digests.jsonl');
    const run = record(dir, '2174935030716737236', recordPath, ['--revision', '999', '--branch', '^/game38/trunk']);
    expect(run.exitCode).toBe(0);
    const [written] = await readRecords(recordPath, BuildDigestRecord);
...
describe('uploadDecision', () => {
  ...
  test('an object at the recorded size with other bytes is refused, and says why', async () => {
    ...
    const decided = await uploadDecision(expected, 22557696, hasher(other).hash);
  ...
  test.each([
    ...
    ['longer', 99999999],
    ['empty', 0],
  ])('an object that is %s is refused without being downloaded', async (_n, bytes) => {
    const existing = hasher(ABC_SHA256);
    const decided = await uploadDecision(expected, bytes, existing.hash);
    expect(decided.action).toBe('refuse');
    expect(decided.action === 'refuse' && decided.detail).toContain(String(bytes));
    expect(existing.calls).toHaveLength(0);
  });
});
...
describe('archiveState', () => {
  ...
  const sizes = (exe: number | null, kfc: number | null): Record<string, number | null> => ({
  ...
  test('a build with no row needs archiving, whatever the bucket holds', () => {
    ...
    expect(decided.state).toBe('missing');
    expect(decided.state === 'missing' && decided.detail).toContain('no digest row');
  });
  ...
  test('an object at another size is a conflict, named with both sizes', () => {
    ...
    expect(decided.state === 'conflict' && decided.detail).toContain('holds 99 bytes, and the record says 3');
  });

  test('a conflict outranks an absent file', () => {
    expect(archiveState('2174935030716737236', row, sizes(null, 99)).state).toBe('conflict');
  });
});
...
describe('asking a loopback bucket', () => {
  ...
  test('a key that is not there reads as absent', async () => {
    expect(await objectSize(bucket, 'build/1/enshrouded_server.kfc')).toBeNull();
  });
  ...
  ])('a request the bucket %s is thrown, named, and carries no credential', async (_name, key) => {
    const names = [
  ...
  test('the sweep covers every recorded build except the one named', async () => {
    ...
    expect(requests.every((request) => request.includes('/build/1/'))).toBe(true);
  });

  test('a sweep over a record holding only the named build asks nothing', async () => {
    requests.length = 0;
    expect(await sweepArchive(bucket, [{ ...row, manifestId: '1' }], '1')).toEqual([]);
...
describe('the emit and append verbs', () => {
  ...
  const run = (args: string[], env: Record<string, string> = {}) =>
    Bun.spawnSync({
      cmd: [process.execPath, 'run', fileURLToPath(new URL('archive.ts', import.meta.url)), ...args],
  ...
  const emittedRow = (out: string): string => out.split('\n').find((text) => text.startsWith('{')) as string;
  ...
  test('emit prints a row and writes no file', async () => {
    const dir = await fetched('2174935030716737236');
    const recordPath = join(await sandbox(), 'build-digests.jsonl');
    const emitted = run(['emit', '--dir', dir, '--manifest', '2174935030716737236', '--record', recordPath]);
    expect(emitted.exitCode).toBe(0);
    expect(await Bun.file(recordPath).exists()).toBe(false);

    const parsed = BuildDigestRecord.parse(JSON.parse(emittedRow(emitted.stdout.toString())));
    expect(parsed.manifestId).toBe('2174935030716737236');
    expect(Object.keys(parsed.files).sort()).toEqual(['enshrouded_server.exe', 'enshrouded_server.kfc']);
  });
  ...
  test('append writes the row emit produced', async () => {
    ...
    const recordPath = join(await sandbox(), 'build-digests.jsonl');
    const emitted = run(['emit', '--dir', dir, '--manifest', '2174935030716737236', '--record', recordPath]);
    const appended = run(['append', '--row', emittedRow(emitted.stdout.toString()), '--record', recordPath]);
  ...
  test('append is quiet and green when the row is already recorded', async () => {
    ...
    const again = run(['append', '--row', JSON.stringify(row), '--record', recordPath]);
  ...
  test('emit refuses bytes the committed row does not pin, and hands nothing on', async () => {
    ...
    const emitted = run(emitArgs(dir, recordPath), { GITHUB_OUTPUT: outputs });
    expect(emitted.exitCode).toBe(1);
    expect(emitted.stdout.toString()).toContain('enshrouded_server.kfc hashes to');
    expect(emitted.stdout.toString()).toContain(`the record says ${ABC_SHA256}`);
  ...
  test.each([
  ...
  ])('status without the read token refuses for %s, and hands on no decision', async (_name, recorded) => {
    const recordPath = join(await sandbox(), 'build-digests.jsonl');
    if (recorded) {
      await appendRecord(recordPath, BuildDigestRecord, row);
    }
    ...
    const asked = run(['status', '--manifest', '2174935030716737236', '--record', recordPath], {
      GITHUB_OUTPUT: outputs,
      R2_ARCHIVE_READ_ACCESS_KEY_ID: '',
      R2_ARCHIVE_READ_SECRET_ACCESS_KEY: '',
    });
    expect(asked.exitCode).toBe(1);
    expect(asked.stdout.toString()).toContain('R2_ARCHIVE_READ_ACCESS_KEY_ID, R2_ARCHIVE_READ_SECRET_ACCESS_KEY');
    expect(await Bun.file(outputs).exists()).toBe(false);
  });
});
...
describe('digestRow', () => {
  ...
  test('the row for the manifest asked for is the one returned', async () => {
    ...
    expect((await digestRow('2174935030716737236', path))?.buildId).toBe('23178631');
  });
  ...
  test('a duplicate is refused whichever manifest is asked for', async () => {
    ...
    await appendRecord(path, BuildDigestRecord, other);
    await expect(digestRow('954904204024183479', path)).rejects.toBeInstanceOf(DuplicateRowError);
  });
});

describe('the status command, against a loopback bucket', () => {
  /** Build 1 is in the bucket at the recorded size. Build 2 is not. */
  const present = ['/loopback/build/1/enshrouded_server.exe', '/loopback/build/1/enshrouded_server.kfc'];
  let server: ReturnType<typeof Bun.serve> | undefined;
  ...
  const status = async (
    ...
    const script = `
      const { commandStatus } = await import(${JSON.stringify(fileURLToPath(new URL('archive.ts', import.meta.url)))});
