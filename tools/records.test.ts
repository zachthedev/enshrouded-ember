...
describe("readRecords", () => {
  test("a file that is not there reads as no rows", async () => {
    ...
    expect(
      await readRecords(join(dir, "absent.jsonl"), SteamBuildRecord),
    ).toEqual([]);
  });
  ...
  test("blank lines are skipped and the rest keep file order", async () => {
    ...
    await Bun.write(
      path,
      `${JSON.stringify(steamRow)}\n\n${JSON.stringify(second)}\n`,
    );
    const rows = await readRecords(path, SteamBuildRecord);
    expect(rows.map((row) => row.branches["public"])).toEqual([
      "23178631",
      "23178632",
    ]);
  });
...
describe("appendRecord", () => {
  test("a row round-trips through the file it was written to", async () => {
    ...
    await appendRecord(path, SteamBuildRecord, steamRow);
    expect(await readRecords(path, SteamBuildRecord)).toEqual([
      steamRow,
      steamRow,
    ]);
  });
  ...
  test("a row that does not match the shape is refused, and nothing is written", async () => {
    ...
    await expect(
      appendRecord(path, SteamBuildRecord, bad),
...
describe("SteamBuildRecord", () => {
  ...
  test.each([
    ...
    "beta.2",
    "b".repeat(64),
  ])("the branch name %p is accepted", (branch) => {
    const value = { ...steamRow, branches: { [branch]: "23178631" } };
...
describe("BuildDigestRecord", () => {
  ...
  test.each(["constructor", "prototype"])(
    "the file name %p is refused by name",
    (fileName) => {
      const value = {
        ...digestRow,
        files: { ...digestRow.files, [fileName]: anyDigest },
  ...
  test("a file named __proto__ is dropped, and cannot empty the row", () => {
    ...
    expect(
      ({} as Record<string, unknown>)["bytes"],
      "reading the row polluted Object.prototype",
    ).toBeUndefined();
    ...
    expect(
      parsed.success && Object.hasOwn(parsed.data.files, "__proto__"),
    ).toBe(false);
  });
  ...
  test.each([
    ...
    expect(BuildDigestRecord.safeParse({ ...digestRow, files }).success).toBe(
      false,
    );
  });
  ...
  test.each([
    ...
    expect(BuildDigestRecord.safeParse({ ...digestRow, branch }).success).toBe(
      want,
    );
  });
});
