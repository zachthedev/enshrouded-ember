...
describe('sliceAppBlock', () => {
  ...
  test('carriage returns do not hide the block', () => {
    ...
    expect(sliceAppBlock(windows, 2278520)).toBe(sliceAppBlock(CAPTURE, 2278520));
  });

  test.each([
    ['no block for that application', CAPTURE, 1203620],
    ['a block that was cut short', CAPTURE.slice(0, CAPTURE.indexOf('"depots"')), 2278520],
...
describe('identityKey', () => {
  ...
  test.each([
    ['a build id moving', { branches: { public: '2' }, manifests: { '10': 'a' } }],
    ['a gid moving', { branches: { public: '1' }, manifests: { '10': 'b' } }],
    ['a branch appearing', { branches: { public: '1', beta: '9' }, manifests: { '10': 'a' } }],
...
describe('recordIfChanged', () => {
  ...
  test.each([
    ...
    ['a depot manifest gid', { manifests: { '2278521': '9999999999999999999' } }],
...
describe('movedBranches', () => {
  ...
  test('a first observation names every branch, because none was known', () => {
    ...
    expect(movedBranches({ record, previous: null, changed: true })).toEqual(['public']);
  });
  ...
  test('a branch named for a property of Object is seen to be new', () => {
    ...
    expect(movedBranches({ record, previous, changed: true })).toEqual(['constructor']);
  });
});
...
describe('stepOutputs', () => {
  ...
  const outputs = (result: WatchResult, depot: string | undefined): Record<string, string> =>
    Object.fromEntries(
  ...
  test('a depot with no public manifest is refused rather than emitted empty', () => {
    expect(() => stepOutputs({ record, previous: null, changed: true }, '9999999')).toThrow(AppInfoError);
  });
  ...
  test('no output line can carry a line break', () => {
    for (const line of stepOutputs({ record, previous: null, changed: true }, '2278521')) {
